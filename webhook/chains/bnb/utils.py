...
def parse_bnb_input_data(input_data):
    ...
    """
    logger.debug(f"تجزیه داده‌های ورودی BNB: {input_data[:10]}...")
    ...
    # بایننس اسمارت چین از EVM استفاده می‌کند، بنابراین شناسه‌های متد مشابه اتریوم هستند
    ...
    # اضافه کردن شناسه‌های متد خاص BEP-20 در صورت نیاز
...
def format_bnb_address(address):
    ...
    # اطمینان از اینکه آدرس با 0x شروع می‌شود (برای BSC)
...
def is_valid_bnb_address(address):
    ...
    # بررسی طول آدرس BSC (0x + 40 کاراکتر هگز)
...
def calculate_bnb_fee(gas_price, gas_used):
    """
    محاسبه کارمزد تراکنش BNB
    ...
    # BNB با 18 رقم اعشار نشان داده می‌شود
    return fee / (10 ** 18)

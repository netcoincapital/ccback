...
from CC.webhook.chains.avax.utils import parse_avax_input_data, calculate_avax_fee, is_token_transfer

logger = get_logger(__name__)

class AvalancheProcessor(TransactionProcessor):
    """پردازشگر وب‌هوک‌های مخصوص آوالانچ (AVAX)"""
    
    def __init__(self):
        super().__init__()
        logger.info("پردازشگر آوالانچ راه‌اندازی شد")
    
    def process_webhook(self, webhook_data):
        """
        پردازش وب‌هوک آوالانچ
        این متد منطق خاص آوالانچ را اجرا می‌کند
        ...
        Returns:
            dict: نتیجه پردازش
        """
        logger.info("پردازش وب‌هوک آوالانچ")
        ...
        if input_data and len(input_data) > 10:
            parsed_data = parse_avax_input_data(input_data)
            webhook_data['avax_input_data'] = parsed_data
        ...
        if 'gasPrice' in webhook_data and 'gasUsed' in webhook_data:
            try:
                ...
                fee = calculate_avax_fee(gas_price, gas_used)
                webhook_data['fee'] = fee
                logger.info(f"کارمزد محاسبه شده برای تراکنش: {fee} AVAX")
        ...
        webhook_data['chain'] = 'avalanche'
        if 'currency' not in webhook_data:
            webhook_data['currency'] = 'avax'
    ...
    def _normalize_webhook_data(self, webhook_data):
        """
        نرمال‌سازی داده‌های وب‌هوک آوالانچ
        بعضی از فیلدها در وب‌هوک آوالانچ ممکن است با فرمت‌های مختلف ارسال شوند
        ...
        if 'amount' in webhook_data and isinstance(webhook_data['amount'], str):
            try:
                ...
                # اگر مقدار برای AVAX در واحد nAVAX است، آن را به AVAX تبدیل کنیم
                if amount > 1e10:  # احتمالاً در واحد nAVAX است
                    amount = amount / 1e18
    ...
    def update_transaction(self, blockchain, tx_hash):
        ...
        """
        logger.info(f"به‌روزرسانی تراکنش آوالانچ: {tx_hash}")
    ...
    def _process_address_transaction(self, blockchain, from_address, to_address, value, transaction_id, webhook_data):
        ...
        Returns:
            dict: نتیجه پردازش
        """
        logger.info(f"پردازش تراکنش آدرس آوالانچ: {transaction_id}")
    ...
    def _process_contract_event(self, blockchain, webhook_data, transaction_id):
        ...
        Returns:
            dict: نتیجه پردازش
        """
        logger.info(f"پردازش رویداد قرارداد آوالانچ: {transaction_id}")

...
def parse_arb_input_data(input_data):
    """
    تجزیه و تحلیل داده‌های ورودی تراکنش آربیتروم
    ...
    """
    logger.debug(f"تجزیه داده‌های ورودی آربیتروم: {input_data[:10]}...")
    ...
    # آربیتروم از EVM استفاده می‌کند، بنابراین شناسه‌های متد مشابه اتریوم هستند
    ...
    # برخی از متدهای خاص آربیتروم
    elif method_id == '0xc6806b40':  # bridgeEth()
        return {
            'method_id': method_id,
            'method_name': 'bridgeEth',
            'type': 'bridge'
        }
...
def format_arb_address(address):
    """
    فرمت‌سازی آدرس آربیتروم (تبدیل به فرمت استاندارد با حروف کوچک)
    
    Args:
        address (str): آدرس آربیتروم
...
def is_valid_arb_address(address):
    """
    اعتبارسنجی آدرس آربیتروم
...
def calculate_arb_fee(gas_price, gas_used):
    """
    محاسبه کارمزد تراکنش آربیتروم
    آربیتروم کارمزد کمتری نسبت به اتریوم دارد
    ...
    # Arbitrum با 18 رقم اعشار نشان داده می‌شود (مانند اتریوم)
    return fee / (10 ** 18)

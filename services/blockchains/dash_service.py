...
class DashService(BaseBlockchainService):
    """Dash blockchain service implementation"""
    
    def __init__(self):
        super().__init__()
        self.logger = get_logger(__file__)
        
        # Initialize Dash node URL
        self.dash_node_url = os.getenv('DASH_NODE_URL')
        if not self.dash_node_url:
            raise RuntimeError("DASH_NODE_URL environment variable is not set")
        ...
        try:
            ...
            tx_details = {
                ...
                "details": {
                    ...
                    "explorer_url": f"https://explorer.dash.org/tx/{transaction_id}",
                    "recipient": recipient,
                    "sender": sender,
    ...
    def send_transaction(self, transaction_id: str) -> Tuple[Dict, Optional[str]]:
        """Send a prepared Dash transaction"""
        try:
            ...
            try:
                ...
                tx.add_output(
                    tx_data['recipient'],
                    int(Decimal(tx_data['amount']) * Decimal(10**8))  # Convert to duffs
                )
                ...
                self.logger.error(f"Error sending transaction via Dash client: {str(e)}")
    ...
    def estimate_fee(self, sender: str, recipient: str, amount: Decimal) -> Tuple[Decimal, Optional[str]]:
        ...
        try:
            ...
            response = requests.get(f"{self.dash_node_url}/fees")
            ...
            # Calculate fee in duffs
            fee_duffs = tx_size * fee_rate
            
            # Convert to DASH
            fee = Decimal(fee_duffs) / Decimal(10**8)
    ...
    def get_balance(self, address: str) -> Tuple[Decimal, Optional[str]]:
        ...
        try:
            # Try Dash node first
            response = requests.get(f"{self.dash_node_url}/address/{address}/balance")
            if response.status_code == 200:
                balance_duffs = response.json()['balance']
                balance = Decimal(balance_duffs) / Decimal(10**8)
                return balance, None
                
            # Fallback to Tatum
            balance, error = self.tatum.get_balance('dash', address)
            if error:
    ...
    def get_transaction_status(self, tx_hash: str) -> Tuple[str, Optional[str]]:
        """Get Dash transaction status"""
        try:
            # Try Dash node first
            response = requests.get(f"{self.dash_node_url}/tx/{tx_hash}")
            if response.status_code == 200:
            ...
            # Fallback to Tatum
            status, error = self.tatum.check_transaction_status('dash', tx_hash)
            if error:
    ...
    def get_transaction_details(self, tx_hash: str) -> Tuple[Dict, Optional[str]]:
        ...
        try:
            ...
            # Fallback to Tatum
            details, error = self.tatum.get_transaction('dash', tx_hash)
            if error:
    ...
    def _get_utxos(self, address: str) -> Tuple[list, Optional[str]]:
        ...
        try:
            response = requests.get(f"{self.dash_node_url}/address/{address}/utxo")
            if response.status_code != 200:

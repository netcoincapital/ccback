...
class DogecoinService(BaseBlockchainService):
    """Dogecoin blockchain service implementation"""
    
    def __init__(self):
        super().__init__()
        self.logger = get_logger(__file__)
        
        # Initialize Dogecoin node URL
        self.dogecoin_node_url = os.getenv('DOGECOIN_NODE_URL')
        if not self.dogecoin_node_url:
            raise RuntimeError("DOGECOIN_NODE_URL environment variable is not set")
    ...
    def prepare_transaction(self, sender: str, recipient: str, amount: Decimal, 
        ...
        """Prepare a Dogecoin transaction"""
        try:
            # Validate addresses
            ...
            tx_details = {
                ...
                "details": {
                    ...
                    "explorer_url": f"https://dogechain.info/tx/{transaction_id}",
                    "recipient": recipient,
                    "sender": sender,
    ...
    def send_transaction(self, transaction_id: str) -> Tuple[Dict, Optional[str]]:
        """Send a prepared Dogecoin transaction"""
        try:
            ...
            try:
                ...
                tx.add_output(
                    tx_data['recipient'],
                    int(Decimal(tx_data['amount']) * Decimal(10**8))  # Convert to koinu
                )
                ...
                self.logger.error(f"Error sending transaction via Dogecoin client: {str(e)}")
    ...
    def estimate_fee(self, sender: str, recipient: str, amount: Decimal) -> Tuple[Decimal, Optional[str]]:
        """Estimate Dogecoin transaction fee"""
        try:
            # Get current fee rate
            response = requests.get(f"{self.dogecoin_node_url}/fees")
            ...
            # Calculate fee in koinu
            fee_koinu = tx_size * fee_rate
            
            # Convert to DOGE
            fee = Decimal(fee_koinu) / Decimal(10**8)
    ...
    def get_balance(self, address: str) -> Tuple[Decimal, Optional[str]]:
        ...
        try:
            # Try Dogecoin node first
            response = requests.get(f"{self.dogecoin_node_url}/address/{address}/balance")
            if response.status_code == 200:
                balance_koinu = response.json()['balance']
                balance = Decimal(balance_koinu) / Decimal(10**8)
                return balance, None
                
            # Fallback to Tatum
            balance, error = self.tatum.get_balance('dogecoin', address)
            if error:
    ...
    def get_transaction_status(self, tx_hash: str) -> Tuple[str, Optional[str]]:
        """Get Dogecoin transaction status"""
        try:
            # Try Dogecoin node first
            response = requests.get(f"{self.dogecoin_node_url}/tx/{tx_hash}")
            if response.status_code == 200:
            ...
            # Fallback to Tatum
            status, error = self.tatum.check_transaction_status('dogecoin', tx_hash)
            if error:
    ...
    def get_transaction_details(self, tx_hash: str) -> Tuple[Dict, Optional[str]]:
        """Get Dogecoin transaction details"""
        try:
            # Try Dogecoin node first
            response = requests.get(f"{self.dogecoin_node_url}/tx/{tx_hash}")
            if response.status_code == 200:
            ...
            # Fallback to Tatum
            details, error = self.tatum.get_transaction('dogecoin', tx_hash)
            if error:
    ...
    def _get_utxos(self, address: str) -> Tuple[list, Optional[str]]:
        ...
        try:
            response = requests.get(f"{self.dogecoin_node_url}/address/{address}/utxo")
            if response.status_code != 200:
    ...
    def _broadcast_transaction(self, raw_tx: str) -> Dict:
        """Broadcast a raw transaction to the Dogecoin network"""
        try:
            response = requests.post(
                f"{self.dogecoin_node_url}/tx/send",
                json={'rawtx': raw_tx}
            )
            if response.status_code != 200:

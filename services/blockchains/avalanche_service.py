...
class AvalancheService(BaseBlockchainService):
    ...
    def __init__(self):
        ...
        self.w3 = Web3(Web3.HTTPProvider(self.avalanche_node_url))
    ...
    def send_transaction(self, transaction_id: str, private_key: str) -> Tuple[Dict, Optional[str]]:
        """Send a prepared Avalanche transaction using Tatum API broadcast endpoint"""
        try:
            ...
            try:
                # Sign transaction
                self.logger.debug("Signing Avalanche transaction with private key")
                ...
                url = f"{self.tatum.base_url}/avalanche/broadcast"
                self.logger.debug(f"Making Avalanche broadcast request to {url}")
                ...
                if response.status_code == 200:
                    ...
                    self.logger.debug(f"Avalanche broadcast response: {result}")
                    ...
                    error_msg = f"Tatum Avalanche broadcast error: {response.status_code} - {response.text}"

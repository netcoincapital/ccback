...
class HistoricalDataService:
    ...
    def get_historical_quotes(self, cmc_ids, time_start=None, time_end=None, interval="daily", fiat="USD"):
        """
        Get historical quotes for specified CMC IDs
    ...
    def store_historical_data(self, currency_ids, time_start=None, time_end=None, interval="daily", fiat_currencies=None):
        """
        Fetch and store historical data for specified currencies
        ...
        try:
            logger.info(f"Starting historical data storage for {len(currency_ids)} currencies")
            ...
            for fiat in fiat_currencies:
                ...
                # Get CMC IDs for API call
                cmc_ids = list(currency_mapping.values())
                
                # Fetch historical quotes
                quotes_data = self.get_historical_quotes(
                    cmc_ids, time_start, time_end, interval, fiat
                )
                
                if isinstance(quotes_data, dict) and quotes_data.get("status") == "error":
                    logger.error(f"Error fetching historical data for {fiat}: {quotes_data.get('message')}")
                    continue
                ...
                try:
                    ...
                    session.commit()
                    logger.info(f"Successfully stored historical data for {fiat}")
                    
                except Exception as db_error:
                    session.rollback()
                    logger.error(f"Database error for {fiat}: {str(db_error)}", exc_info=True)
                    fail_count += 1
                finally:
                    session.close()
            ...
            return {
                "success": True,
                "message": f"Historical data storage completed. {success_count} records added, {fail_count} failed",
